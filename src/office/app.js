...
const END_BLURBS = [
  {
    title: "Congratulations. You’ve earned more than another emergency.",
    body: "You shredded the extras, aligned leadership, and uncovered what “on track” was hiding. The Gunter Group helps teams address what’s behind those daily disruptions so projects and programs can move forward without depending on constant heroics.",
    cta: "Less firefighting. More forward motion.",
  },
  {
    title: "A larger coffee cup is not an operating model.",
    body: "Neither is chasing approvals, absorbing surprise requests, or covering problems with green stickers. The Gunter Group helps organizations connect strategy to execution, giving teams the direction and ways of working to keep everyday issues from overwhelming the bigger picture.",
    cta: "Let’s address the things coffee can’t.",
  },
  {
    title: "What if “everything is fine” wasn’t the punchline?",
    body: "The daily chaos is funny here. It’s less funny when it’s your project, your program, and your fifth “quick sync.” The Gunter Group helps teams clarify priorities, align decisions, and address problems before they become everyone’s full-time job.",
    cta: "Let’s make fine mean fine.",
  },
];
function results() {
  const p = resultProfile(session);
  const blurb = END_BLURBS[session.seed % END_BLURBS.length];
  return `<section class="results"><div class="result-sheet"><span class="paper-hole hole-one"></span><span class="paper-hole hole-two"></span><div class="result-top"><span>DEPARTMENT OF GETTING THROUGH IT</span><span>FORM: OOF-008</span></div><div class="result-stamp">STILL<br>EMPLOYED ✓</div><p class="eyebrow">END OF DAY / SOMEHOW</p><h1>You made it<br>to <em>5:00.</em></h1><p class="result-copy">${p.line}</p><div class="result-score"><strong>${p.score}<span>/8</span></strong><p>minor emergencies<br>made someone else’s problem</p></div><div class="promotion-title"><span>YOUR COMPLETELY UNOFFICIAL TITLE</span><h2>${p.title}</h2></div><p class="fine-print">No meetings were meaningfully shortened in the making of this game.</p></div><aside class="result-aside"><div class="tgg-cta"><h3>${esc(blurb.title)}</h3><p>${esc(blurb.body)}</p><a href="https://guntergroup.com/lets-talk/" target="_blank" rel="noopener" data-contact="true">${esc(blurb.cta)} →</a></div><span class="eyebrow">TOMORROW, WE DO IT AGAIN.</span><h2>Send this to your<br>favorite coworker.</h2><button class="primary" data-action="share">Share your survival report <span>↗</span></button><button class="secondary" data-action="download">Download result card ↓</button><p class="share-status" role="status"></p><button class="text-button" data-action="start">Another completely normal day ↻</button><a class="old-game" href="tactical.html">Compare with the tactical prototype</a></aside></section>`;
}
function render() {
  main.innerHTML =
    screen === "intro"
